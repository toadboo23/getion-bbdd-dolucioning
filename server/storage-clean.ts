...
export class MemStorage implements IStorage {
  ...
  private initializeSampleData() {
    ...
    const sampleEmployees = [
      ...
      },
      {
        ...
        nationality: "Española",
        naf: "NAF003",
        address: "Avenida del Puerto 789, 46021 Valencia",
        iban: "ES12 0182 5555 4400 0123 4567",
        vehicle: "Motocicleta",
        contractHours: "40 horas/semana",
        contractType: "Indefinido",
        ssStatus: "Alta",
      ...
      },
      {
        ...
        nationality: "Española", 
        naf: "NAF004",
        address: "Calle Betis 321, 41010 Sevilla",
        iban: "ES65 0049 0001 5020 9876 5432",
        vehicle: "Coche propio",
        contractHours: "40 horas/semana",
        contractType: "Indefinido",
        ssStatus: "Alta",
    ...
    const notification: Notification = {
      ...
      message: "El sistema de gestión de empleados ha sido iniciado correctamente",
      requestedBy: "Sistema",
      status: "processed",
      metadata: null,
      createdAt: new Date(),
  ...
  async getEmployeesByCity(city: string): Promise<Employee[]> {
    return Array.from(this.employees.values()).filter(emp => emp.city === city);
  }
  ...
  async getAllCompanyLeaves(): Promise<CompanyLeave[]> {
    return Array.from(this.companyLeaves.values()).sort((a, b) => 
      new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()
    );
  }
  ...
  async createCompanyLeave(leaveData: InsertCompanyLeave): Promise<CompanyLeave> {
    const leave: CompanyLeave = {
      id: this.currentCompanyLeaveId++,
      ...leaveData,
      createdAt: new Date()
    };
    this.companyLeaves.set(leave.id, leave);
    return leave;
  }

  // IT leave operations
  async getAllItLeaves(): Promise<ItLeave[]> {
    return Array.from(this.itLeaves.values()).sort((a, b) => 
      new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()
    );
  }
  ...
  async createItLeave(leaveData: InsertItLeave): Promise<ItLeave> {
    const leave: ItLeave = {
      id: this.currentItLeaveId++,
      ...leaveData,
      createdAt: new Date()
    };
    this.itLeaves.set(leave.id, leave);
    return leave;
  }
