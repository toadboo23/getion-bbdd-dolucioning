...
-- Actualizar la función get_captation_dashboard con tipos de datos corregidos
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(SUM(CASE WHEN e.status = 'active' THEN e.horas ELSE 0 END), 0)::INTEGER as horas_fijas_actuales,
        ...
        ) THEN e.horas ELSE 0 END), 0)::INTEGER as horas_fijas_pendientes,
        GREATEST(COALESCE(chr.horas_fijas_requeridas, 0) - COALESCE(SUM(CASE WHEN e.status = 'active' THEN e.horas ELSE 0 END), 0), 0)::INTEGER as deficit_horas_fijas,
        COUNT(*)::INTEGER as total_empleados_activos,
        COUNT(CASE WHEN e.status = 'active' THEN 1 END)::INTEGER as empleados_activos,
        COUNT(CASE WHEN e.status = 'it_leave' THEN 1 END)::INTEGER as empleados_baja_it,
        CASE 
